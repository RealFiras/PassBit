...
The popup is a compact Arabic RTL security panel with a dark background, lock-shield logo, one password field, two live measurement tiles, one direct score ring derived from the entropy calculation, one breach-status line, and a collapsed generator link. The optional encrypted favorites vault opens as a separate view so it does not crowd the main check. The main screen avoids decorative cards and technical paragraphs.
