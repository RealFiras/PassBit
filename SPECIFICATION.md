...
The popup is a compact Arabic RTL security panel with a dark background, lock-shield logo, one password field, two live measurement tiles, one direct score ring derived from the entropy calculation, and one breach-status row. The generator is a labeled expandable panel with explicit actions: generate, generate another, copy, or use for analysis. The optional encrypted favorites vault opens as a separate view so it does not crowd the main check. The main screen avoids ambiguous icons and technical paragraphs.

| UI region | Behavior | Accessibility expectation |
| --- | --- | --- |
| Header | Shows PassBit, version 1.5.0, and the lock-shield logo. | Brand text remains available independently of the icon. |
